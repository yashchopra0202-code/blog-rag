...
date: 
scraped_at: 2026-09-04T13:15:37+00:00
---

August 21, 2026

Maria Despoina Siampou and Shushman Choudhury, Research Scientists, Google Research
...
To demonstrate this complementary capability, we introduce
...
To build a model with a deeper understanding of places, we must distinguish between a place's

(its name and category) and its

(its aggregated visit footprint). In prior geospatial AI research works, mobility patterns were almost exclusively applied to predicting the next POI a user will visit. In contrast, the ME-POIs framework shifts mobility from an output prediction task to an input feature that defines the place itself.
...
Rather than discarding textual descriptions, the ME-POIs framework enriches them. It does so by aligning high-level language embeddings (the standard vector representations extracted from advanced models like Gemini) with the newly generated mobility vectors by maximizing their

. We layer the mobility signal directly on top of the language representation, creating a more holistic view of the place or business.
...
To establish a baseline, we compared the ME-POIs framework to standard text-only embedding models (like

), existing trajectory-based geospatial models (like

), and hybrid variations to isolate exactly how much value the mobility patterns added to the equation.
...
Beyond outperforming standard baselines, one of the most notable findings emerged when comparing a model trained

against those that had access

. In several cases, like in price-level classification, the mobility-only model surpassed the text-only language models. This reveals a fascinating and often underused truth about urban dynamics: our collective actions at a physical place are often far more descriptive than the formal words used to label it.
...
Ultimately, ME-POIs lay the groundwork for AI models that truly understand the rhythm of our cities. It is also part of our broader

effort to create geospatial models and datasets that turn planetary data into actionable intelligence.
